...
with st.container():
    ...
    d2 = col1.number_input('Die 2', min_value=1, max_value=6, value=1, step=1, icon='🎲', width=150)
    d3 = col1.number_input('Die 3', min_value=1, max_value=6, value=1, step=1, icon='🎲', width=150)
    roll = [d1, d2, d3]

    num_rolls = col2.number_input('Number of rolls', min_value=1, max_value=3, value=1, step=1, width=150)
    num_games = col2.selectbox('Number of games', [10, 100, 1000, 10000, 100000], index=1, width=150)

    col2.space()
    if col2.button('Estimate probabilities'):
        estimate = True
        st.session_state['estimated'] = True
        st.session_state['wins'], st.session_state['mids'], st.session_state['losses'] = gg.run_simulation(roll=roll, num_rolls=num_rolls, num_games=num_games)
